...
def plot_categorical_columns(df):
    ...
    for col in df.select_dtypes(include=['object']):
        df[col].value_counts().plot(kind='bar', figsize=(11,5))
        plt.show()
...
def group_values(df,col1,agg1,col2):
    ...
    """
    df.groupby(col1).agg({col2:agg1}).plot(kind='bar', figsize=(11,4))
...
group_values(weather_df,'Weather','max','Visibility (km)')
group_values(weather_df,'Weather','min','Visibility (km)')
